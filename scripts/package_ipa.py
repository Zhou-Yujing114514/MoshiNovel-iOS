...
app_path = sys.argv[1] if len(sys.argv) > 1 else "Payload/MoshiNovel.app"
output = sys.argv[2] if len(sys.argv) > 2 else "MoshiNovel.ipa"

with zipfile.ZipFile(output, 'w', zipfile.ZIP_DEFLATED) as zf:
    zf.writestr('Payload/', '')
    zf.writestr('Payload/MoshiNovel.app/', '')
